...
import (
	...
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/roblox/assets"
	...
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/roblox/publish"
...
func Reupload(ctx *context.Context, r *request.Request) {
	...
	uploadAsset := func(wg *sync.WaitGroup, assetInfo *develop.AssetInfo, location string) {
		...
		uploadHandler, err := publish.NewUploadImageHandler(client, assetInfo.Name, assetData, groupID)
		if err != nil {
		...
		res := <-uploadQueue.QueueTask(func() (int64, error) {
			return retry.Do(
				...
				func(try int) (int64, error) {
					...
					switch err {
					case publish.UploadImageErrors.ErrNotAuthenticated:
						clientutils.GetNewCookie(ctx, r, "cookie expired")
					case publish.UploadImageErrors.ErrModerated:
