...
import (
	...
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/app/assets/shared/assetutils"
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/app/assets/shared/clientutils"
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/app/assets/shared/uploaderror"
	...
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/atomicarray"
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/retry"
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/roblox/assetdelivery"
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/roblox/develop"
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/roblox/games"
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/roblox/ide"
	"github.com/Timspects/roblox-kartfr-asset-reuploader-modified-by-timspects/internal/shardedmap"
...
func imageRateLimitBackoff(try int) time.Duration {
	if try < 1 {
		try = 1
	}
	if try > imageUploadRateLimitMaxPower {
		try = imageUploadRateLimitMaxPower
	}
...
func Reupload(ctx *context.Context, r *request.Request) {
	...
	uploadAsset := func(wg *sync.WaitGroup, assetInfo *develop.AssetInfo, location string) {
		...
		res := <-uploadQueue.QueueTask(func() (int64, error) {
			return retry.Do(
				retry.NewOptions(retry.Tries(imageUploadRetryTries)),
				func(try int) (int64, error) {
					...
					switch err {
					case ide.UploadImageErrors.ErrNotLoggedIn:
						clientutils.GetNewCookie(ctx, r, "cookie expired")
					case ide.UploadImageErrors.ErrInappropriateName:
						assetInfo.Name = fmt.Sprintf("(%s) [Censored]", assetInfo.Name)
					default:
						if errors.Is(err, ide.ErrRateLimited) && try < imageUploadRetryTries {
							time.Sleep(imageRateLimitBackoff(try))
						}
